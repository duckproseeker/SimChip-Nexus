...
export function LiveRunNode({ data, selected }: NodeProps) {
  ...
  return (
    <div className={`rounded-lg border-2 bg-white dark:bg-zinc-900 px-4 py-3 min-w-[160px] shadow-sm ${selected ? 'border-blue-500' : 'border-zinc-200 dark:border-zinc-700'}`}>
      <Handle type="target" position={Position.Left} id="project" style={{ top: '20%' }} />
      <Handle type="target" position={Position.Left} id="scenario" style={{ top: '35%' }} />
      <Handle type="target" position={Position.Left} id="map" style={{ top: '50%' }} />
      <Handle type="target" position={Position.Left} id="weather" style={{ top: '65%' }} />
      <Handle type="target" position={Position.Left} id="sensor" style={{ top: '80%' }} />
      <div className="text-xs font-semibold uppercase tracking-wide text-zinc-400 mb-1">实时仿真</div>
