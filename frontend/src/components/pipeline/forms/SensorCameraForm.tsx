...
export function SensorCameraForm({ data, onChange }: Props) {
  return (
    <div className="flex flex-col gap-3">
      {textField('sensor_id', '传感器 ID', 'camera_front', data, onChange)}
      ...
      <div className="text-xs font-semibold text-zinc-400 uppercase tracking-wide">图像</div>
      {numField('width', '宽度 (px)', data, onChange, 1)}
      {numField('height', '高度 (px)', data, onChange, 1)}
      {numField('fov', '视角 (°)', data, onChange, 1)}
    </div>
  );
}
